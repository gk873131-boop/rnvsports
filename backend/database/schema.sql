-- ============================================================
-- RNV Sports - Complete Database Schema
-- DB: salvaind_rnvsports | Engine: InnoDB | Charset: utf8mb4
-- ============================================================
...
CREATE TABLE IF NOT EXISTS tbl_category (
  ...
  status          TINYINT DEFAULT 1 COMMENT '0=deleted',
...
CREATE TABLE IF NOT EXISTS tbl_product (
  ...
  special             TINYINT DEFAULT 0 COMMENT 'Hot selling',
  featured_product    TINYINT DEFAULT 0,
  new_arraival        TINYINT DEFAULT 0 COMMENT 'Typo preserved from PHP original',
...
-- ─── Price (Size / Age Tiers) ────────────────────────────────
CREATE TABLE IF NOT EXISTS tbl_price (
  price_id      INT AUTO_INCREMENT PRIMARY KEY,
  product_id    INT NOT NULL,
  age           VARCHAR(100) COMMENT 'Size label or age group',
...
CREATE TABLE IF NOT EXISTS tbl_customer (
  ...
  customer_password   VARCHAR(255) COMMENT 'bcrypt hash',
  customer_password1  VARCHAR(255) COMMENT 'Deprecated plaintext column — do not use',
...
CREATE TABLE IF NOT EXISTS tbl_cart (
  ...
  product_cookie  VARCHAR(255) COMMENT 'Guest cart identifier',
  cart_price      DECIMAL(10,2),
  size            VARCHAR(100),
  size_id         INT,
  product_color   VARCHAR(100),
  color_id        INT,
  status          TINYINT DEFAULT 1 COMMENT '0=removed',
  created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (product_id) REFERENCES tbl_product(product_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS tbl_wishlist (
  ...
  whislist_price  DECIMAL(10,2) COMMENT 'Typo preserved from PHP original',
  status          TINYINT DEFAULT 1,
  created_at      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (product_id) REFERENCES tbl_product(product_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS tbl_order_management (
  ...
  order_no        VARCHAR(50)         COMMENT 'Shared across all items in one order',
  ...
  order_status    VARCHAR(50)         DEFAULT 'Pending'
                  COMMENT 'Pending|Processing|Dispatched|Delivered|Cancelled',
  date            DATE,
  mobile          VARCHAR(20),
  email           VARCHAR(255),
  ...
  city            VARCHAR(100),
  payment_status  TINYINT DEFAULT 0   COMMENT '0=Pending 1=Success',
  ...
  shipp_address   TEXT                COMMENT 'Legacy duplicate column',
...
CREATE TABLE IF NOT EXISTS tbl_promo (
  ...
  discount    DECIMAL(10,2)       COMMENT 'Percentage discount',
  expiry_date DATE,
  status      TINYINT DEFAULT 1,
  created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS tbl_offer (
  ...
  expiry_date     DATE,
  amount          DECIMAL(10,2)   COMMENT 'Minimum order trigger amount',
...
CREATE TABLE IF NOT EXISTS tbl_video (
  id      INT AUTO_INCREMENT PRIMARY KEY,
  video1  VARCHAR(500) COMMENT 'YouTube embed URL',
  video2  VARCHAR(500) COMMENT 'YouTube embed URL'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS tbl_employee (
  ...
  user_id           VARCHAR(100) UNIQUE COMMENT 'Login username',
  password          VARCHAR(255)       COMMENT 'bcrypt hash',
...
CREATE TABLE IF NOT EXISTS tbl_purchase (
  ...
  supplier_id    INT,
  item_id        INT COMMENT 'FK to tbl_product',
  qty            INT,
  rate           DECIMAL(10,2),
  amount         DECIMAL(10,2),
  purchase_date  DATE,
...
CREATE INDEX IF NOT EXISTS idx_product_category     ON tbl_product(category_id);
CREATE INDEX IF NOT EXISTS idx_product_brand        ON tbl_product(brand_id);
CREATE INDEX IF NOT EXISTS idx_product_special      ON tbl_product(special);
CREATE INDEX IF NOT EXISTS idx_product_featured     ON tbl_product(featured_product);
CREATE INDEX IF NOT EXISTS idx_product_new_arraival ON tbl_product(new_arraival);
CREATE INDEX IF NOT EXISTS idx_cart_cookie          ON tbl_cart(product_cookie);
CREATE INDEX IF NOT EXISTS idx_cart_user            ON tbl_cart(user_id);
CREATE INDEX IF NOT EXISTS idx_order_no             ON tbl_order_management(order_no);
CREATE INDEX IF NOT EXISTS idx_order_user           ON tbl_order_management(user_id);
CREATE INDEX IF NOT EXISTS idx_order_date           ON tbl_order_management(date);
CREATE INDEX IF NOT EXISTS idx_wishlist_user        ON tbl_wishlist(user_id);
...
INSERT IGNORE INTO tbl_employee
  (employee_name, employee_email, employee_type, user_id, password, emp_status, status)
VALUES
  ('Administrator', 'admin@rnvsports.co.in', 'admin', 'admin',
   '$2b$12$GmVbAhf0CpPjh7v5TGGfQOFyWf2i4/4Wx4WZ1mZ4j4K5KK6cTPbKe', 'Active', 1);

-- Initial video slots row
INSERT IGNORE INTO tbl_video (id, video1, video2) VALUES (1, '', '');
